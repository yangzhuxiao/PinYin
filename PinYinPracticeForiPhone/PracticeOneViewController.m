...
@interface PracticeOneViewController () < UICollectionViewDelegate, UICollectionViewDataSource, UIScrollViewDelegate >
...
@property (nonatomic, copy) PracticeOneCollectionViewCell *selectedCell;
...
- (void)setUpCollectionViewAndPageControl
{
    ...
    [_itemCollectionView registerClass:[PracticeOneCollectionViewCell class] forCellWithReuseIdentifier:@"ItemCell"];
...
- (void)toneIsSelected:(id)sender {
    ...
    if ([sender tag] >= 10) {
        _tempSecondTone = [sender titleLabel].text;
    } else if ([sender tag] < 10) {
        _tempFirstTone = [sender titleLabel].text;
    }
}

- (void)confirmSelection:(id)sender {
    if ([_tempFirstTone isEqualToString:_firstTone]
        && [_tempSecondTone isEqualToString:_secondTone]) {
        _selectedCell.congratulateLabel.hidden = NO;
        _correctNumber ++;
    } else {
        _selectedCell.righAnswerLabel.hidden = NO;
    }
    _selectedCell.confirmSelectionButton.userInteractionEnabled = NO;
    [self updateCountLabel];
}
...
{
    PracticeOneCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"ItemCell" forIndexPath: indexPath];
    
    // must do this, otherwise the two buttons will be seen !
    _selectedCell.confirmSelectionButton.userInteractionEnabled = YES;
    cell.congratulateLabel.hidden = YES;
    cell.righAnswerLabel.hidden = YES;
    
    // store the right tones for latter comparing use
    _firstTone = _tonesArray[indexPath.row][0];
    _secondTone = _tonesArray[indexPath.row][1];
    
    NSArray *pinYinArrayWithoutTone = [_pinYinWithoutToneArray[indexPath.row] componentsSeparatedByString:@" "];
